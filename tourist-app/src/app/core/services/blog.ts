...
export class BlogService {
  ...
  getBlogs(page = 1, pageSize = 10): Observable<PagedResult<BlogEntry>> {
  ...
  addComment(blogId: number, request: CreateCommentRequest): Observable<BlogComment> {
    return this.http.post<BlogComment>(`${this.apiUrl}/${blogId}/comments`, request, {
      headers: this.getAuthHeaders()
    });
  }

  updateComment(blogId: number, commentId: number, request: UpdateCommentRequest): Observable<BlogComment> {
    return this.http.put<BlogComment>(`${this.apiUrl}/${blogId}/comments/${commentId}`, request, {
      headers: this.getAuthHeaders()
    });
  }
